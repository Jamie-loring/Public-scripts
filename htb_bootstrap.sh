...
run_installation() {
  ...
  [ "$USER_SETUP" = "true" ] && TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 2: User Creation
  [ "$USER_SETUP" = "true" ] && TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 3: PAM/PW-less
  [ "$SHELL_ENVIRONMENT" = "true" ] && TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 4: Shell
  ...
  [ "$WORDLISTS" = "true" ] && TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 6: Wordlists
  
  if [ "$REPOS_ESSENTIAL" = "true" ] || [ "$REPOS_PRIVILEGE" = "true" ]; then
    TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 7: Repos
  fi
  
  [ "$FIREFOX_EXTENSIONS" = "true" ] && TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 8: Firefox
  [ "$AUTOMATION_SCRIPTS" = "true" ] && TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 9: Scripts
  [ "$USER_SETUP" = "true" ] && TOTAL_PHASES=$(( TOTAL_PHASES + 1 )) # Phase 10: Disable Old User
  ...
  [ "$USER_SETUP" = "true" ] && {
    echo -e "${CYAN}║ ${phase_num}. **Passwordless Login Configuration** ${CYAN}║${NC}"
    phase_num=$(( phase_num + 1 ))
  }
  [ "$SHELL_ENVIRONMENT" = "true" ] && {
  ...
  [ "$USER_SETUP" = "true" ] && {
    echo -e "${CYAN}║ ${phase_num}. Disable Original User Account                ${CYAN}║${NC}"
    phase_num=$(( phase_num + 1 ))
  }
...
phase10_pam_config() {
    ...
    if ! grep -q 'pam_succeed_if.so user ingroup nopasswdlogin' /etc/pam.d/common-auth; then
        ...
        # Insert the 'pam_succeed_if' line just before the 'pam_deny.so' line
        # The success=1 skips the next module (pam_unix.so, which asks for the password)
...
# ============================================
# PHASE 9: DISABLE ORIGINAL USER ACCOUNT (PREVENTS OLD USER LOGIN)
# ============================================
phase9_disable_old_user() {
...
# ============================================
# PHASE 3: SHELL ENVIRONMENT (Now Phase 4 in the run_installation call)
# ============================================
phase3_shell_setup() {
  log_progress "Phase: Shell Environment (Zsh + Oh-My-Zsh + p10k)"
  log_info "Pre-configuring Zsh and Oh-My-Zsh for $USERNAME"
...
EOF
  ...
  echo "2. The system should now auto-login to '${USERNAME}'. If not, you can log in instantly by selecting the user and pressing ENTER."
